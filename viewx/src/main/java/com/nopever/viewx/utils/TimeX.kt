 ...
 * @return [Int] [Calendar.SUNDAY]
 */
fun getDateWeekByTime(timestamp: Long): String {
    val calendar = Calendar.getInstance()
    calendar.time = Date(timestamp)
    val get = calendar.get(Calendar.DAY_OF_WEEK)
    var weekDayString = ""
    weekDayString = when (get) {
        Calendar.SUNDAY -> "星期日"
        Calendar.MONDAY -> "星期一"
 ...
 * @return [Int] [Calendar.SUNDAY]
 */
fun getDateWeekDayByTime(timestamp: Long): String {
    val calendar = Calendar.getInstance()
    calendar.time = Date(timestamp)
    val get = calendar.get(Calendar.DAY_OF_WEEK)
    var weekDayString = ""
...
/**
 * 判断日期字符串是几天前、几天后，还是今天
 * @param format 日期格式，默认 yyyy-MM-dd
 * @return "今天", "x天前", "x天后" (或者解析失败返回原字符串)
 */
@RequiresApi(Build.VERSION_CODES.O)
fun String.toRelativeDayDescription(format: String = "yyyy-MM-dd"): String {
    return try {
        ...
        when {
            daysDiff == 0L -> "今天"
            daysDiff > 0L -> "${daysDiff}天后"
            else -> "${abs(daysDiff)}天前"
        }
    } catch (e: Exception) {
        // 如果解析出错（比如格式不对），直接返回原字符串或空字符串，看需求
        this
    }
}
